...
cols.cast <- reactive({ #this is the color data, by ID
  if(!is.null(cols())){
    suppressMessages(arrange(left_join(filter(labs(),Type!="skip"),
                                       cols()),Battery.ID))
  }
})
```
...
renderDygraph(withProgress(
  ...
  dygraph.cast(imported.xy(),input$bottom.xn,input$top.yn,input$bottom.xn,
               input$top.yn,cols.cast()$Color,cols()$Color,group="brc")
))
```
...
renderDygraph(withProgress(
  ...
  dygraph.cast(imported.xy(),input$bottom.xn,input$bottom.yn,input$bottom.xn,
               input$bottom.yn,cols.cast()$Color,cols()$Color,group="brc")
))
```
...
xyline1 <- reactive(if(!is.null(imported.xy())) {
  gg <- linegraph(imported.xy(),input$bottom.xn,input$top.yn) + 
    scale_fill_manual(values=cols.cast()$Color,name="")
  if(input$xy.avg) gg <- gg + scale_color_manual(values=cols()$Color,name="") #avg/sem
  else gg <- gg + scale_color_manual(values=cols.cast()$Color,name="") #regular
  gg
})
xyline2 <- reactive(if(!is.null(imported.xy())) {
  gg <- linegraph(imported.xy(),input$bottom.xn,input$bottom.yn) + 
    scale_fill_manual(values=cols.cast()$Color,name="")
  if(input$xy.avg) gg <- gg + scale_color_manual(values=cols()$Color,name="") #avg/sem
  else gg <- gg + scale_color_manual(values=cols.cast()$Color,name="") #regular
  gg
