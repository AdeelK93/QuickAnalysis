...
* Reporting time data in seconds is preferred over H:M:S  
...
totaltime.sorted <- reactive(if(!is.null(imported.orig())) {
  ...
  if(min(x[x>0],na.rm=T)<0.05) { #time in days?
    x <- unlist(tapply(x,imported.orig()$Filename,function(x) {
...
cycle.sorted <- reactive(if(!is.null(imported.orig())) {
  rowMeans(imported.orig()[input$cycle],na.rm=T)
})
...
```{r import.vars}
fluidPage(column(12,div(style = "height:70px;",
  htmlOutput("voltage"),
  htmlOutput("current"),
  htmlOutput("amphours"),
...
imported <- reactive({
  ...
  x <- suppressMessages(left_join(x,labs()))
  mutate(x, Type=as.factor(Type),Battery.ID=as.factor(Battery.ID))
})
```
...
observeEvent(labs(), {
  values$colors <- data.frame(Type=sort(unique(labs()$Type)),
    Color=rainbow_hcl(length(unique(labs()$Type)),70), stringsAsFactors=F)
})
observeEvent(imported.orig(), {
  values$colors <- data.frame(Type=sort(unique(labs()$Type)),
    Color=rainbow_hcl(length(unique(labs()$Type)),70), stringsAsFactors=F)
...
output$colors.hot <- renderRHandsontable(if(!is.null(values$colors)){
  rhandsontable(values$colors, contextMenu=F, maxRows=length(unique(labs()$Type)),
                rowHeaders = NULL) %>%
...
cols.cast <- reactive({
  if(!is.null(cols())){
    suppressMessages(arrange(left_join(labs(),cols()),Battery.ID))
  }
})
```

Explore {data-orientation=rows data-navmenu="Continuous"}
=====================================
...
imported.xy <- reactive(if(!is.null(imported())) {
  ...
  else if(input$xy.cycle==0) return(imported())
  else {
    imported()[imported()$Cycle==input$xy.cycle,] %>%
...
### Top Graph
```{r top.graph}
renderDygraph(withProgress(message="Rendering plot...",
                          dygraph.cast(imported.xy(),input$bottom.xn,input$top.yn,
                          input$bottom.xn,input$top.yn,cols.cast()$Color,group="brc")))
```
...
```{r bottom.graph}
renderDygraph(withProgress(message="Rendering plot...",
                           dygraph.cast(imported.xy(),input$bottom.xn,input$bottom.yn,
                           input$bottom.xn,input$bottom.yn,cols.cast()$Color,group="brc")))
```
...
fluidPage(
  ...
  splitLayout(
    textInput("xypub.savew","Width:","12"),
    textInput("xypub.saveh","Height:","8"),
...
fluidPage(
  ...
  splitLayout(
    textInput("xypub.x1","X start:"),
    textInput("xypub.x2","X end:")
  ),
  splitLayout(
    textInput("xypub.y1","Y1 start:"),
    textInput("xypub.y2","Y1 end:")
  ),
  splitLayout(
  ...
  splitLayout(
    ...
    textInput("xyhline.val","Position:","0"),
    textInput("xyhline.size","Size:","0.8")
  )
)
```
...
xyline1 <- reactive(if(!is.null(imported.xy())) {
  linegraph(imported.xy(),input$bottom.xn,input$top.yn) + 
    scale_color_manual(values=cols.cast()$Color,name="")
})
xyline2 <- reactive(if(!is.null(imported.xy())) {
  linegraph(imported.xy(),input$bottom.xn,input$bottom.yn) + 
    scale_color_manual(values=cols.cast()$Color,name="")
})
renderPlot(withProgress(message="Rendering...",xypub.graph()))
xypub.graph <- reactive(if(!is.null(imported.xy())) {
  xypub.y1 <- as.numeric(input$xypub.y1) #these are the x-y ranges
  xypub.y2 <- as.numeric(input$xypub.y2)
  xypub.y1b <- as.numeric(input$xypub.y1b)
  xypub.y2b <- as.numeric(input$xypub.y2b)
  xypub.x1 <- as.numeric(input$xypub.x1)
  xypub.x2 <- as.numeric(input$xypub.x2)
  
  if(!is.na(xypub.y1) & !is.na(xypub.y2) & !is.na(xypub.x1) & !is.na(xypub.x2)) {
    gg <- xyline1() + coord_cartesian(xlim=c(xypub.x1,xypub.x2),ylim=c(xypub.y1,xypub.y2))
  } else gg <- xyline1()
  if(input$xypub.both){
    if(!is.na(xypub.y1b) & !is.na(xypub.y2b) & !is.na(xypub.x1) & !is.na(xypub.x2)) {
      gg2 <- xyline2() + coord_cartesian(xlim=c(xypub.x1,xypub.x2),ylim=c(xypub.y1b,xypub.y2b))
    } else gg2 <- xyline2()
  ...
  if(input$xyhline.col!="None" & input$xyhline.col!="none" & nchar(input$xyhline.col)){
    gg <- gg + geom_hline(aes_(yintercept=as.numeric(input$xyhline.val)),linetype=2,
                          color=input$xyhline.col,size=as.numeric(input$xyhline.size))
  }
...
output$xypub.png <- downloadHandler(paste('plot-', Sys.Date(), '.png', sep=''),
   content = function(file) if(!is.null(imported.xy())) {
     ggsave(file,device="png",width=as.numeric(input$xypub.savew),
            height=as.numeric(input$xypub.saveh))
   }
)
output$xypub.svg <- downloadHandler(paste('plot-', Sys.Date(), '.svg', sep=''),
   content = function(file) if(!is.null(imported.xy())) {
     ggsave(file,device="svg",width=as.numeric(input$xypub.savew),
            height=as.numeric(input$xypub.saveh))
   }
)
```
...
fluidPage(
  ...
  textInput("options.yv","Value of the known variable: (numeric, or type min/max)"),
  htmlOutput("varnames.xn"),
  checkboxInput("avg","Calculate average/SEM by type"),
...
fluidPage(column(12,div(style = "height:80px;",
...
imported.filtered <- reactive(if(!is.null(imported())){
  ...
  x <- imported()
  if(input$filter.step) x <- filter(imported(),Step==input$filter.step)
  tryCatch(filter(x,
...
output$protocol.ids <- renderUI({ 
  selectInput("protocol.ids","Filter prediction by Battery ID:",
              c("All",unique(imported()$Battery.ID)))
  })
fluidPage(
...
bar <- reactive(if(!is.null(reduced())) bargraph(reduced(),options.name,
        options.name.orig)+scale_fill_manual(values=cols()$Color,name=""))
line <- reactive(if(!is.null(reduced())) dygraph.cast(reduced(),
        "Cycle",options.name,"Cycle",options.name.orig,
        cols.cast()$Color,cols()$Color))
ggline <- reactive(if(!is.null(reduced())) {
  gg <- linegraph(reduced(),"Cycle",options.name)+ylab(options.name.orig) +
    scale_fill_manual(values=cols()$Color,name="")
  if(input$avg) gg <- gg + scale_color_manual(values=cols()$Color,name="") #avg/sem
  else gg <- gg + scale_color_manual(values=cols.cast()$Color,name="") #regular
  gg
})
...
shinyApp(
  ...
  server = function(input, output, session){
    output$bar <- renderPlotly(withProgress(message="Calculating...",tryCatch(ggplotly(bar()) %>%
          config(displayModeBar = F),error=function(e) ggplotly(ggplot()))))
...
fluidPage(
  ...
  helpText("Download chart options:"),
  splitLayout(
    textInput("pub.savew","Width:","12"),
    textInput("pub.saveh","Height:","8"),
...
fluidPage(
  helpText("Include axis ranges:"),
  splitLayout(
    textInput("pub.x1","X start:"),
    textInput("pub.x2","X end:")
  ),
  splitLayout(
    textInput("pub.y1","Y start:"),
    textInput("pub.y2","Y end:")
  ),
  helpText("Horizontal line options:"),
  splitLayout(
    textInput("hline.col","Color:","None"),
    textInput("hline.val","Position:","0"),
    textInput("hline.size","Size:","0.8")
  ),
...
pub.graph <- reactive(if(!is.null(reduced())) {
  pub.y1 <- as.numeric(input$pub.y1) #these are the x-y ranges
  pub.y2 <- as.numeric(input$pub.y2)
  pub.x1 <- as.numeric(input$pub.x1)
  pub.x2 <- as.numeric(input$pub.x2)
  
  if(input$pub.type=="Bar"){
    if(!is.na(pub.y1) & !is.na(pub.y2)){
      gg <- bar() + coord_cartesian(ylim=c(pub.y1,pub.y2))
    } else gg <- bar()
  } 
  else { #line
    if(!is.na(pub.y1) & !is.na(pub.y2) & !is.na(pub.x1) & !is.na(pub.x2)){
         gg <- ggline() + coord_cartesian(xlim=c(pub.x1,pub.x2),ylim=c(pub.y1,pub.y2))
       } else gg <- ggline()
  }
  
  if(input$hline.col!="None" & input$hline.col!="none" & nchar(input$hline.col)){
    gg <- gg + geom_hline(aes_(yintercept=as.numeric(input$hline.val)),linetype=2,
                          color=input$hline.col,size=as.numeric(input$hline.size))
  }
...
output$pub.png <- downloadHandler(paste('plot-', Sys.Date(), '.png', sep=''),
   content = function(file) if(!is.null(reduced())) {
     ggsave(file,device="png",width=as.numeric(input$pub.savew),
            height=as.numeric(input$pub.saveh))
   }
)
output$pub.svg <- downloadHandler(paste('plot-', Sys.Date(), '.svg', sep=''),
   content = function(file) if(!is.null(reduced())) {
     ggsave(file,device="svg",width=as.numeric(input$pub.savew),
            height=as.numeric(input$pub.saveh))
   }
)
```
