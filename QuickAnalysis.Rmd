...
xypub.graph <- reactive(if(!is.null(imported.xy())) {
  ...
  if(!is.na(input$xypub.y1) & !is.na(input$xypub.y2) & !is.na(input$xypub.x1) & !is.na(input$xypub.x2)) {
    gg <- xyline1() + coord_cartesian(xlim=c(input$xypub.x1,input$xypub.x2),
...
pub.graph <- reactive(if(!is.null(reduced())) {
  ...
  if(input$pub.type=="Bar"){
    if(!is.na(input$pub.y1) & !is.na(input$pub.y2)){
      gg <- bar() + coord_cartesian(ylim=c(input$pub.y1,input$pub.y2))
    } else gg <- bar()
  } 
  else { #line
    if(!is.na(input$pub.y1) & !is.na(input$pub.y2) & !is.na(input$pub.x1) & !is.na(input$pub.x2)){
         gg <- ggline() + coord_cartesian(xlim=c(input$pub.x1,input$pub.x2),
